...
> ルール判定はキーワードと時間だけで決めるため、文脈の読み違いは起こります
> （業務影響と私用の区別は語句ベースで補正しているものの、たとえば私用の言葉を含む
> 仕事の連絡などは取りこぼします）。文面の意図を踏まえた並べ替えには
