...
同居している優先度インボックス（`/`、`/api` 配下）にも `POST /ask` にも一切依存していません。
...
`public/dining/js/data.js` の店舗・料理人・体験は **すべて架空** です。
実在店舗について未確認の情報を書かないための方針です。エリア（銀座・丸の内・麻布台・
日本橋・京都）と価格帯のみ現実に即しています。

写真も同様の理由で使っていません。`public/dining/js/art.js` が各店のパレットから
...
`src/app.js` の `public/` 静的配信にそのまま乗るため、専用ルートは追加していません。
優先度インボックス側（`/`、`/api`、`/ask`）のコードには一切手を入れていません。
